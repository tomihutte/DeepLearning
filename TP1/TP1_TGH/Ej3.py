...
start = time.time()
n = 100  # cantidad de datos a testear entre 1-10.000
...
for k in np.arange(0, 10, 1):
    print(k)
    y_pred = D.predict(x_test[:n], (k + 1) * 10)
    l.append(np.mean(y_pred == y_test[:n].ravel()))
    print("El porcentaje de aciertos usando k={}, es {}%".format(k + 1, l[k] * 100))
